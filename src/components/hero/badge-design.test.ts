...
describe('badge design fallbacks', () => {
  it('디자인을 생략해도 모든 필수 색상을 제공한다', () => {
    expect(resolveBadgeDesign()).toMatchObject({ backgroundColor: '#fafafa', textColor: '#252629', borderColor: 'transparent', strapColor: '#696b70', strapBorderColor: '#696b70', strapPattern: undefined });
  });
  ...
  it('명시한 테두리와 배경 이미지는 기본값으로 덮어쓰지 않는다', () => {
    const strapPattern = { src: '/strap.svg', repeatLength: 108 };
    expect(resolveBadgeDesign({ strapColor: '#123456', strapBorderColor: '#abcdef', strapPattern, borderColor: '#e60027', backgroundImage: '/badge.svg' })).toMatchObject({ strapBorderColor: '#abcdef', strapPattern, borderColor: '#e60027', backgroundImage: '/badge.svg' });
  });
